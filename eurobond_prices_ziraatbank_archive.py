...
for date in dates:
    ...
    time.sleep(1)
    driver.find_element("id", "datePicker").send_keys(date.strftime("%d.%m.%Y"))
    time.sleep(2)
    driver.find_element(
        "xpath",
        '//*[@id="content-1"]/div/div[1]/div/div[2]/a',
    ).click()
    time.sleep(3)

    file_name = "ziraatbank_eurobond_" + date.strftime("%d-%m-%Y") + ".csv"
